...
capitalization was already applied in earlier sections where required.
...
* This specification does not mandate any single,
...
Several weak recommendations related to Search Goal possibilities are included,
but they are conditional on specific intents (for example, compatibility with other methodologies)
and are not considered universal enough yet.
...
as it makes it easier to plan execution of multiple benchmarks one-by-one on the same SUT.
...
This situation is quite common in pactice, when the DUT is the boottleneck
for a traffic consisting of small frames, but achieves the line rate with large frames.
...
but the Controller MAY repeat the same Trial Inputs to get multiple independent Trial Outputs.
...
but also as a basis for several related quantities not tied to a single Trial.
...
the Measurer is not performing a direct Forwarding Rate measurement.
...
This is an OPTIONAL attribute. If present, this value MUST be positive.
...
as an ordered list of Goal Results that in that case MUST appear in the exact same order
...
Compliance with RFC 2544 is governed by {{RFC2544}};
rules of MLRsearch Specification cannot change that,
but they can give recommendations to improve comparability.

The following Search Goal instance is RECOMMENDED to make the corresponding
...
equal to each other, and the value is the Throughput.

Goal Duration Sum smaller than Goal Final Trial Duration
should have no effect on the results.
A non-zero Goal Exceed Ratio would needlessly prolong the Search
when Short Trials of both loss types are present.

The Goal Loss Ratio and Goal Exceed Ratio
...
Small enough Goal Duration Sum prevents MLRsearch
from repeating zero-loss Full-Length Trials.
Allowing those would make it less clear
whether the result is compliant with {{Section 24 of RFC2544}}
or whether it is a new, stricter methodology.
...
is RECOMMENDED (for comparability reasons) to get a compatible Search Result:
