...
For specification purposes, it is convenient to define a *base quantity* first,
describing the commonalities for a larger set of variables.
Subsequently, several *narrower quantities* are defined
by referring to their base quantity and adding conditions
specific for the one narrow role the variable fulfills in the specification.
...
MLRsearch needs a base quantity useful not only as an input for a Trial,
but also as a basis for several narrower quantities based on selected Trials.
...
: Load is an intersection of Intended Load and Constant Load.

  Similarly to those definitions, this quantity
...
of a newly defined narrower quantity based on Load.
...
Bsesides Trial Forwarding Rate, the Conditional Throughput will be another example
of a newly defined narrower quantity based on Forwarding Rate,
that is why Throughput is also listed here.
