...
are outside the scope of the MLRsearch Specification.
    ...
    In a situation where there is a risk DUT is the bottleneck,
    but at the same time also a hope that it is not (e.g., achieving line rate without losses),
    and the test is performed to distinguish those possibilities.
    I could add a sentece here or to Max Load section,
    but currently I do not think it would be needed.
...
the instances SHOULD contain all such additional attributes.
    ...
    Controller implementations can either pass instances to Manager or not.
    If passing, unfamiliar attributes can be either copied or filtered out,
    if not passing then it does not matter (DNM) whether values would be included.
    In either case, Controller implementation could try
    to do other decisions based on unfamiliar values or not (ignore them).
    Both old and new paragraphs either say SHOULD (S), imply SHOULD NOT (N),
    or do not unambiguously say (?) for including and ignoring separately.
    Let me create a table:
    pass | include | ignore | old | new
    -----+---------+--------+-----+----
    yes  | yes     | yes    | ??  | SS
    yes  | yes     | no     | ??  | SN
    yes  | no      | yes    | ??  | NS
    ...
    no   | DNM     | yes    | ?S  | ?S
    no   | DNM     | no     | ?N  | ?N
    Many software libraries can copy "unrecognized" attributes correctly.
    I can tell you the new meaning was always my intention,
    but "except when passing" phrase did not really carry it in the "no" branches.
    So yes, this is a strengthening of draft, but I do not think it is controversial.
...
The value MUST be positive.
    ...
    Conditionally, if Lower Bound can exist, the code implies the Trial Duration will be reached.
    Unconditionally, Min Load might become an Upper Bound, in which case
    Controller is allowed to bail out without reaching this Trial Duration value.
