...
- The definition of Throughput is not clear when Trial Results are inconsistent
  ...
  yields smaller Trial Loss Ratio, Throughput can no longer be pinned
...
mainly the Binary Search for unconditionally compliant {{RFC2544}} Throughput.
...
with {{RFC2544}} Throughput methodology, is excessively slow due to two main factors.
...
non-zero Goal Loss Ratio for their Load search.
...
It is easy to modify the Binary Search to find a Lower Bound
...
models and computations of TCP performance in presence of packet loss.
...
The Binary Search never encounters inconsistent Trials.
...
* Treatment of situations where Offered Load is found to differ noticeably from Intended Load
  is not prescribed, only the "use smaller Max Load" recommendation is mentioned.
...
The MLRsearch Specification prescribes Regular Goal Results
...
In physics, a single magnitude can be expressed in multiple ways
as a product of a *unit* and a *value*.
This document uses the computer science vocabulary,
where the term quantity can describe multiple variables or constants at once.

One *instance* of a scalar quantity is called a *value*,
...
In rare cases, a *primary unit* is mentioned,
...
References to several central terms from the above documents
are given in the following subsections.
Some of those terms need no further discussion,
but some require some elaboration to put into proper context.
...
on commodity x86/ARM CPUs (vs. purpose-built ASICs, NPUs, and FPGAs).
  ...
  When the MLRsearch Specification mentions SUT configuration,
...
are outside the scope of the MLRsearch Specification.
    ...
    COMMENT: https://github.com/rfc-editor/AUTH48-rfc9971/pull/47#discussion_r3585803296
    > When that SHOULD can be safely ignored?
    When DUT happens to have good performance. For example,
    when line rate is reached without loss, the line itself becomes the bottleneck,
    but that does not make SUT badly designed.
...
Several existing quantities describe the intensity of the traffic entering a SUT.
MLRsearch needs a quantity useful not only as an input for a Trial,
but also as a basis for several related quantities not tied to a single Trials.

The following subsections refer to existing terms
before introducing their combination used as such a base quantity in this document.
...
: For specification purposes, Load is also assumed to be Constant Load.
  ...
  Similarly to existing definitions, this quantity
is related to one (input or output) SUT interface.
...
### Forwarding Rate Quantities

Similar to Load as a main quantity acting as an input for a Trial,
Forwarding Rate is an existing quantity describing an output from a Trial.

Besides Trial Forwarding Rate, the Conditional Throughput will be another example
of a newly defined quantity based on Forwarding Rate.
...
The methodology of measuring Throughput is codified
in "Throughput" ({{Section 26.1 of RFC2544}}).

This document frequently uses the phrase *{{RFC2544}} Throughput* when referring
not only to the Throughput quantity itself, but also to the methodology requirements for benchmarking it.
...
MAY be described by a single Trial Load value
...
in sufficient enough detail to imply how the Trial Forwarding Ratio should be computed.
...
and the Trial Forwarding Rate is 0.7 million fps.
  ...
  Because Trial Forwarding Rate is anchored to the Load defined for one interface,
...
used to collectively describe multiple Trial Output instances.
  ...
  Measurer implementations MAY provide additional attributes.
The Controller implementations SHOULD ignore any such optional attribute
they are not familiar with.
If a Controller implementation passes selected Trial Outputs to the Manager,
the instances SHOULD contain all such additional attributes.

    COMMENT: https://github.com/rfc-editor/AUTH48-rfc9971/pull/47#discussion_r3585856566
    > This is different from the OLD behavior.
    > Optional attributes can be passed only when supported. I suggest we revert this change
    Here is the github initial paragraph:
    > Measurer implementations may provide additional optional attributes.
    > The Controller implementations SHOULD ignore values of any optional attribute
    > they are not familiar with, except when passing Trial Output instances to the Manager.

    Explanation generated by Gemini-3.5 Flash:

    Thanks for catching this. The goal of this change isn't
    to invent a new architectural requirement,
    but rather to fix an ambiguity in the original phrasing
    that actually distorted the intent.

    **The Problem with the OLD Text:**
    The old text said: *"...SHOULD ignore values...
    except when passing Trial Output instances to the Manager."*
    Grammatically, applying an "except" to a "SHOULD ignore" creates a double negative.
    It inadvertently implied that if a Controller passes data to the Manager,
    it is *forbidden* from ignoring those attributes
    (meaning it would be *required* to process or validate them).
    That was never our intent.

    **The Intent of the NEW Text:**
    We want to decouple two distinct behaviors for handling unfamiliar attributes:
    1. **Processing:** The Controller should not try to interpret or act on
    attributes it doesn't recognize. It should ignore them.
    2. **Forwarding (Telemetry):** If the Controller decides to pass a Trial Output
    to the Manager for telemetry, it shouldn't strip out those unfamiliar attributes.
    It should preserve them so the Manager can see them.

    The new text separates these into two clean, independent `SHOULD` statements.
    It is technically a clarification and tightening of the draft,
    but it aligns exactly with standard telemetry forwarding practices
    (preserving unrecognized fields) without accidentally forcing the Controller to parse them.
...
Measurer is not performing a direct Forwarding Rate measurement.
...
The value MUST be positive.
    ...
    COMMENT: https://github.com/rfc-editor/AUTH48-rfc9971/pull/47#discussion_r3585865721
    > I still see this NEW deviating from the use in the main text:
    > "Certain Trials must reach this minimum.."
    > I would revert to OLD
    Conditionally (in discussion), if Lower Bound can exist, the code an appendix
    implies that Trial Duration value will be reached.
    Unconditionally (in definition), there is a possibility
    Min Load might become an Upper Bound, in which case
    Controller is allowed to bail out without reaching that Trial Duration value.
    I think the current text is good enough, but let me know if you see an improvement.
...
and those Trial Results can be enough for classification as an Upper Bound.
  ...
  If such shorter Trials prove no Lower Bound exists,
this Trial Duration value may never be reached, thus shortening Search Duration.
...
when focusing on this Search Goal.
  ...
  Note that shorter Trial Durations might still be selected by the Controller,
...
might also require users to specify "how quickly" Trial Durations should increase.

  Refer to ["Compliance"](#compliance) for important Search Goals.
...
In practice, the Load frequently becomes a Lower Bound instead.
...
In practice, the Load frequently becomes an Upper Bound instead.
...
without accessing data outside the instance.
...
Search Result unconditionally compliant with {{Section 24 of RFC2544}}.
    ...
    COMMENT: https://github.com/rfc-editor/AUTH48-rfc9971/pull/47#discussion_r3585926962
    > why this isn't MUST? when it can be ignored?
    > Unless we clarify that, my pref is to revert the change
    The non-zero value of exceed ratio does not have bad enough consequences,
    it is merely impractical, not non-compliant.
    The last paragraph in this subsection already states other values are not disallowed,
    that is why MUST in the first paragraph would be wrong.
...
single-ratio Binary Search in {{RFC2544}} and walks through the
...
Each bound in Binary Search is "hard",
...
The meaning of a Goal Duration Sum is that
...
and ["Loss Inversion"](#loss-inversion) for specifics when comparing different Load values.
