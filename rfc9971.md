...
Some of them are used in definitions of further MLRsearch Specification terms,
some are used only for discussion purposes, for example contrasting to other terms.
...
The primary definition, as copied from "Intended load (Iload)" ({{Section 3.5.1 of RFC2285}}).
...
: This definition does not distinguish a per-interface load from aggregate load.

  For purposes of MLRsearch Specification, Intended Load will refer to a single input interface.

  Intended Load is the quantity controlled by the *tester* (external source).

### Offered Load

The primary definition, as copied from "Offered load (Oload)" ({{Section 3.5.2 of RFC2285}}).
...
: This definition is not explicit about how many SUT input interfaces are involved.

  For purposes of MLRsearch Specification, Offered Load will refer to a single input interface.

  Offered Load value is not directly controlled by the external source,
only indirectly via its control over Intended Load.
...
and ["Intended Load"](#intended-load),
in the sense that all three definitions specify that this value
applies to one (input or output) SUT interface.
As neither of those older definitions list all properties relevant
for MLRsearch Specification, the new term Load was chosen.
...
The primary definition, as copied from "Forwarding rate (FR)" (Section 3.6.1 of {{RFC2285}}).
...
: While the definition mentions a destination interface, presumably on the tester side,
this quantity is related to a single output interface of a DUT.

  Obviously, a measured value of Forwarding Rate will depend on the Load.

  For MLRsearch Specification purposes, each Forwarding Rate value
is measured during a Trial, and different Trials MAY measure different values
even at the same Load and duration.

### Throughput

The primary definition, as copied from "Throughput" ({{Section 3.17 of RFC1242}}).
...
in "Throughput" ({{Section 26.1 of RFC2544}}), but that does not change the original definition.

  Indirectly, {{RFC2544}} implies the Throughput value MUST come from a zero-loss Trial
that SHOULD have long enough duration.

  Implicitly, this is either an Offered Load measured at a specific input interface,
or a Forwarding Rate measured at a specific output interface.
The difference only becomes important in imbalanced scenarios,
for example as described in "Partially meshed traffic" ({{Section 3.3.2 of RFC2285}}).
Due to the zero loss condition, the two measurements lead to the same value in balanced scenarios.
...
to distinguish it from later derived quantities that are not tied that way.
  ...
  For specification purposes, it is assumed that this is
a "Constant Load" ({{Section 3.4 of RFC1242}}) by default.
